  ...
  <div class="addBrand-container">
    <div class="container">
      <el-form ref="ruleForm"
        ...
        <div>
          <el-form-item label="Included Dishes:"
                        required>
            <el-form-item>
              <div class="addDish">
        ...
        <div>
          <el-form-item label="Combo Image:"
                        required
                        prop="image">
            <image-upload :prop-image-url="imageUrl"
                          @imageChange="imageChange">
              Max size 2M<br>PNG JPEG JPG only<br>Recommended size 200x200 or 300x300
            </image-upload>
          </el-form-item>
        </div>
        <div class="address">
          <el-form-item label="Description:">
            <el-input v-model="ruleForm.description"
                      ...
                      :rows="3"
                      maxlength="200"
                      placeholder="Combo description, max 200 characters" />
          </el-form-item>
        </div>
    ...
    return {
      name: {
        ...
        validator: (rule: any, value: string, callback: Function) => {
          if (!value) {
            callback(new Error('Combo name is required'))
          } else {
            const reg = /^([A-Za-z0-9一-龥]){2,20}$/
            if (!reg.test(value)) {
              callback(new Error('Combo name must be 2-20 characters'))
            } else {
              callback()
            }
          }
      ...
      price: {
        ...
        validator: (rules: any, value: string, callback: Function) => {
          ...
          if (!reg.test(value) || Number(value) <= 0) {
            callback(
              new Error(
                'Invalid price. Enter a positive number with up to 2 decimal places'
              )
            )
          } else {
            callback()
    ...
    ;(this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        if (this.dishTable.length === 0) {
          return this.$message.error('Please add at least one dish to the combo')
        }
        if (!this.ruleForm.image) return this.$message.error('Combo image is required')
        ...
        if (this.actionType == 'add') {
          ...
          editSetmeal(prams)
            .then(res => {
              if (res.data.code === 1) {
                this.$message.success('Combo updated successfully!')
                this.$router.push({ path: '/setmeal' })
              } else {
